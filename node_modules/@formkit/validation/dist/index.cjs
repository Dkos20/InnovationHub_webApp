 ...
 */
const validatingMessage = core.createMessage({
    type: 'state',
    blocking: true,
    visible: false,
    value: true,
...
function createValidationPlugin(baseRules = {}) {
    return function validationPlugin(node) {
        ...
        let observedNode = observer.createObserver(node);
        ...
        function reboot(newValidation, newRules) {
            var _a;
            if (utils.eq(Object.keys(propRules || {}), Object.keys(newRules || {})) &&
                utils.eq(validation, newValidation))
                return;
            ...
            observer.removeListeners(observedNode.receipts);
            ...
            observedNode.kill();
            observedNode = observer.createObserver(node);
...
function run(current, validations, node, state, removeImmediately, complete) {
    ...
    function next(async, result) {
        ...
        observer.applyListeners(node, observer.diffDeps(validation.deps, newDeps), () => {
    ...
    if ((!utils.empty(node.value) || !validation.skipEmpty) &&
        ...
        if (utils.empty(node.value) && validation.skipEmpty && state.isPassing) {
...
function createCustomMessage(node, validation, i18nArgs) {
    const customMessage = node.props.validationMessages &&
        utils.has(node.props.validationMessages, validation.name)
    ...
    return [
        ...
        [hintGroups[1], hintGroups[2], hintGroups[3]].reduce((hints, group) => {
            ...
            if (hasDebounce.test(group)) {
                ...
                group
                    .split('')
                    .forEach((hint) => utils.has(map, hint) && Object.assign(hints, map[hint]));
            }
            return hints;
        }, { name: rule }),
    ];
